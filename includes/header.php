...
$metaImage = (string) ($pageImage ?? ($currentScheme . '://' . $host . '/img/f799f37016151c7eff761f955aea5006.jpg'));
...
$organizationJsonLd = [
    ...
    'logo' => $currentScheme . '://' . $host . '/img/f799f37016151c7eff761f955aea5006.jpg',
...
<head>
    ...
    <script>
        // Configuration Tailwind — thème « Mairie de Rufisque-Est »
        // preflight: false → ne réinitialise pas les styles existants (cohabite avec style.css legacy)
        tailwind.config = {
            darkMode: 'class',
    ...
    <link rel="stylesheet" href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>assets/css/style.css">
    <style>
    ...
    <link rel="icon" type="image/svg+xml" href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>assets/img/pwa-icon-192.svg">
    <link rel="apple-touch-icon" href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>assets/img/pwa-icon-192.svg">
...
<body class="font-sans antialiased text-slate-800 dark:text-slate-100 dark:bg-slate-950">
...
<?php
$maireNavLinks = [
    ['url' => 'index.php', 'label' => 'Accueil', 'match' => ['index.php']],
    ['url' => 'maire.php', 'label' => 'M. le Maire', 'match' => ['maire.php']],
    ['url' => 'services.php', 'label' => 'Services', 'match' => ['services.php']],
    ['url' => 'documents.php', 'label' => 'Documents', 'match' => ['documents.php']],
    ['url' => 'consultations.php', 'label' => 'Consultations', 'match' => ['consultations.php', 'consultation.php']],
    ['url' => 'conseil-municipal.php', 'label' => 'Conseil municipal', 'match' => ['conseil-municipal.php']],
    ['url' => 'paiements.php', 'label' => 'Payer en ligne', 'match' => ['paiements.php', 'payer.php', 'paiement-retour.php']],
    ['url' => 'actualites.php', 'label' => 'Actualités', 'match' => ['actualites.php', 'actualite.php']],
    ['url' => 'projets.php', 'label' => 'Projets', 'match' => ['projets.php']],
];
if ($maireNavOffres) {
    $maireNavLinks[] = ['url' => 'offres.php', 'label' => 'Offres', 'match' => ['offres.php']];
}
$maireNavLinks[] = ['url' => 'contact.php', 'label' => 'Contact', 'match' => ['contact.php']];
?>
    ...
    <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between gap-4 h-16">
            ...
            <a href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>index.php"
               class="brand flex items-center gap-2 text-xl font-bold text-mairie-800 dark:text-mairie-300 hover:text-mairie-600 dark:hover:text-mairie-200 transition-colors">
                <span class="w-9 h-9 rounded-xl bg-gradient-to-br from-mairie-700 to-mairie-900 flex items-center justify-center text-white shadow-glow">
                ...
                <span class="hidden sm:block">Rufisque<span class="text-gold-500">-Est</span></span>
            </a>

            <!-- Navigation desktop -->
            <nav id="mainNav" class="nav hidden lg:flex items-center gap-1 flex-1 justify-center">
                <?php foreach ($maireNavLinks as $link):
                    ...
                    <a href="<?php echo htmlspecialchars($urlPrefix . $link['url'], ENT_QUOTES, 'UTF-8'); ?>"
                       class="<?php echo $isActive ? 'active bg-mairie-50 dark:bg-mairie-900/30 text-mairie-800 dark:text-mairie-200' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'; ?> px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap">
            ...
            <div class="nav-actions flex items-center gap-2">
                <?php if ($maireCitoyenConnecte): ?>
                    <a href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>citoyen/profil.php"
                       class="hidden md:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-mairie-50 dark:bg-mairie-900/40 text-mairie-800 dark:text-mairie-200 text-sm font-semibold hover:bg-mairie-100 dark:hover:bg-mairie-900/60 transition-colors">
                        <span class="w-6 h-6 rounded-full bg-mairie-800 text-white flex items-center justify-center text-xs">👤</span>
                        <?php echo htmlspecialchars($maireCitoyenPrenom !== '' ? $maireCitoyenPrenom : 'Mon espace', ENT_QUOTES, 'UTF-8'); ?>
                    </a>
                <?php elseif ($maireComptesCitoyensActifs): ?>
                    <a href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>citoyen/connexion.php"
                       class="hidden md:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-mairie-800 to-mairie-700 text-white text-sm font-semibold hover:from-mairie-700 hover:to-mairie-600 shadow-glow transition-all hover:-translate-y-0.5">
                        Espace citoyen
                    </a>
                <?php endif; ?>
                <button class="theme-toggle p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" id="themeToggle" aria-label="Activer le mode sombre" title="Mode sombre / clair">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
                </button>
                <button class="menu-toggle lg:hidden p-2 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" id="menuToggle" aria-label="Ouvrir le menu" aria-expanded="false">
        ...
        <div id="mobileMenu" class="lg:hidden hidden pb-4 pt-2 border-t border-slate-200 dark:border-slate-800">
            <nav class="flex flex-col gap-1">
                <?php foreach ($maireNavLinks as $link):
                    $isActive = in_array($currentPage, $link['match'], true); ?>
                    <a href="<?php echo htmlspecialchars($urlPrefix . $link['url'], ENT_QUOTES, 'UTF-8'); ?>"
                       class="<?php echo $isActive ? 'bg-mairie-50 dark:bg-mairie-900/30 text-mairie-800 dark:text-mairie-200' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'; ?> px-3 py-2 rounded-lg text-sm font-medium transition-colors">
                        <?php echo htmlspecialchars($link['label'], ENT_QUOTES, 'UTF-8'); ?>
                    </a>
                <?php endforeach; ?>
                <?php if ($maireCitoyenConnecte): ?>
                    <a href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>citoyen/profil.php"
                       class="mt-2 px-3 py-2 rounded-lg bg-mairie-50 dark:bg-mairie-900/40 text-mairie-800 dark:text-mairie-200 font-semibold flex items-center gap-2">
                        <span>👤</span>
                        <?php echo htmlspecialchars($maireCitoyenPrenom !== '' ? $maireCitoyenPrenom : 'Mon espace', ENT_QUOTES, 'UTF-8'); ?>
                    </a>
                <?php elseif ($maireComptesCitoyensActifs): ?>
                    <a href="<?php echo htmlspecialchars($urlPrefix, ENT_QUOTES, 'UTF-8'); ?>citoyen/connexion.php"
                       class="mt-2 px-3 py-2 rounded-lg bg-gradient-to-r from-mairie-800 to-mairie-700 text-white font-semibold text-center">
                        Espace citoyen
                    </a>
                <?php endif; ?>
            </nav>
...
<script>
    // Menu mobile : toggle au clic sur le bouton hamburger
    (function () {
