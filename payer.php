                ...
                <form method="POST" action="payer.php?service=<?php echo urlencode($serviceCode); ?>" class="space-y-4">
                    ...
                    <?php if (!$citoyenConnecte): ?>
                        <div>
                            ...
                            <input type="text" id="nom" name="nom" required value="<?php echo htmlspecialchars($prefill['nom'], ENT_QUOTES, 'UTF-8'); ?>" maxlength="120" class="w-full px-3 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:border-mairie-500 focus:ring-2 focus:ring-mairie-200 dark:focus:ring-mairie-900 outline-none transition">
                        </div>
                        <div class="grid sm:grid-cols-2 gap-3">
                            <div>
                                <label for="telephone" class="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-1.5">Téléphone *</label>
                                <input type="tel" id="telephone" name="telephone" required value="<?php echo htmlspecialchars($prefill['telephone'], ENT_QUOTES, 'UTF-8'); ?>" maxlength="40" placeholder="+221 77 ..." class="w-full px-3 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:border-mairie-500 focus:ring-2 focus:ring-mairie-200 dark:focus:ring-mairie-900 outline-none transition">
                            </div>
                            <div>
                                <label for="email" class="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-1.5">Email (facultatif)</label>
                                <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($prefill['email'], ENT_QUOTES, 'UTF-8'); ?>" maxlength="190" class="w-full px-3 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:border-mairie-500 focus:ring-2 focus:ring-mairie-200 dark:focus:ring-mairie-900 outline-none transition">
                            </div>
                        </div>
                    <?php endif; ?>
                    ...
                    <div>
                        ...
                        <textarea id="details" name="details" rows="2" maxlength="2000" placeholder="Ex : pour le compte de M. Diop · date de réservation souhaitée · numéro d'extrait" class="w-full px-3 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:border-mairie-500 focus:ring-2 focus:ring-mairie-200 dark:focus:ring-mairie-900 outline-none transition resize-y"><?php echo htmlspecialchars($prefill['details'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                    </div>

                    <div>
                        <p class="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Moyen de paiement</p>
                        <div class="grid gap-2">
                            <?php foreach (MAIRE_PAIEMENT_PROVIDERS as $code => $lbl):
                                $checked = $code === $prefill['provider'];
                                $providerIcons = ['orange_money' => '🟠', 'wave' => '🌊', 'log' => '🧪'];
                                $icon = $providerIcons[$code] ?? '💳';
                            ?>
                                <label class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 hover:border-mairie-500 dark:hover:border-mairie-400 cursor-pointer transition-all">
